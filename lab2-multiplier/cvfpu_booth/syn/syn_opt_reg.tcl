...
set file_handle [open "prj.txt" r]
set sources [split [read $file_handle] "\n"]
close $file_handle

foreach file_el $sources {
    if {[string match "*vhd*" $file_el] || [string match "*tb*" $file_el]} {
        analyze -work WORK -format "VHDL"  [string cat "../tb/" $file_el]
    } else {
        analyze -work WORK -format "SVERILOG"  [string cat "../src/" $file_el]
    }
...
#TEST clock = 0 TODO
create_clock -name MY_CLK -period 2.2 clk_i
...
report_area > ../reports/fpnew_area_opt_reg.txt
report_timing > ../reports/fpnew_timing_opt_reg.txt

write_sdf ../netlist/fpnew.sdf 
write_sdc ../netlist/fpnew.sdc 
write -f verilog -hierarchy -output ../netlist/fpnew.v
