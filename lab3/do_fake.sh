...
SYN_SCRIPT_FILE="$SYN_DIR/syn_fake.tcl"
##########################################
...
cmd_init(){
	...
	if find "$SIM_DIR" -name "wave_fake.do" -print -quit | grep -q .; then
    	echo "do wave_fake.do" >> "$SIM_SCRIPT_FILE"
	fi
	echo 'puts "\n########## SIMULATION STARTS ##########\n"' >> $SIM_SCRIPT_FILE 
...
cmd_sim() {
	...
	if [ $GUI == nogui ]; then
		vsim -t ps -c -L "$SIM_DIR/mem_wrap" -work "$SIM_DIR"/work "$SIM_DIR"/work."$SIM_TOP_LVL_ENTITY" -do "$SIM_SCRIPT_FILE" -voptargs=+acc 
		...
		vsim -t ps -L "$SIM_DIR/mem_wrap" -work "$SIM_DIR"/work "$SIM_DIR"/work."$SIM_TOP_LVL_ENTITY" -do "$SIM_SCRIPT_FILE" -voptargs=+acc 
	else
...
cmd_sim_post_syn() {
    ...
    vlog -svinputport=relaxed -work "$SIM_WORK_DIR" "./netlist/riscv_core.v"
    ...
    # Simulate the $SIM_TOP_LVL_ENTITY entity design using sim.do tcl script
	if [ $GUI == nogui ]; then
		vsim -t ps -c -L "$SIM_DIR/mem_wrap" -suppress 3009 -L /eda/dk/nangate45/verilog/qsim2020.4 -work "$SIM_DIR"/work "$SIM_DIR"/work."$SIM_TOP_LVL_ENTITY" -do "$SIM_SCRIPT_FILE" -voptargs=+acc 
    	#vsim -work "$SIM_WORK_DIR" -c -sv_seed random -onfinish stop -voptargs=+acc -do "$SIM_SCRIPT_FILE" "$SIM_TOP_LVL_ENTITY"
	elif [ $GUI == gui ]; then
    	#vsim -work "$SIM_WORK_DIR" -sv_seed random -onfinish stop -voptargs=+acc -do "$SIM_SCRIPT_FILE" "$SIM_TOP_LVL_ENTITY"
		vsim -t ps -L "$SIM_DIR/mem_wrap" -suppress 3009 -L /eda/dk/nangate45/verilog/qsim2020.4 -work "$SIM_DIR"/work "$SIM_DIR"/work."$SIM_TOP_LVL_ENTITY" -do "$SIM_SCRIPT_FILE" -voptargs=+acc 
	else
		echo "Error: $2 is a wrong parameter"; usage; exit 1;
	fi
...
cmd_syn() {
    ...
    rm -rf "RISCV_rep_fake"
    mkdir -p "RISCV_rep_fake"
